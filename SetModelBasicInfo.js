var myHeaders = new Headers();
myHeaders.append("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6IlV0aHAwdjVsUnpOc1lZT3VyQ3JTdmciLCJ0eXAiOiJhdCtqd3QifQ..ACxAuIWqHvR7Az0YUdXl6RbYhGctMVTm18_8sSbbxqkSiCosg81cZ7PTf9dT-e-iiRrJ_Shx8CibZ-f1UpH3xFz-OtowNXE-6q3rIg50hPFlwoaxN-WPsgpZ46OoxKc_BKvKMRyidvpnBikgZqQ9mFz7w0YjV_JWIcoX8pKnCEATsxiH8-C3KH1-4P-w0BTeXhSXJWSJUo4Xq5kGc12HWghzpL7HR3ySqeZ2Hp2AfQVYwQlkRjyw4vUYSfhxF_nqsX6PFgVaBQ3u0BnAK2AHBrzJMWzjbc74j8--pABqf8I2QMChc5Yl06J-paK2yw9p4PfV9eE1-BeG8jpCauDBKA");
myHeaders.append("tenantId", "3a0c7870-e7fe-ea61-eb01-0d6d2bf0e045");
myHeaders.append("Content-Type", "application/json");

var raw = JSON.stringify({
   "ModelFile": "E:\\DevCoreService\\2023\\TestModel\\2DBlueBeach\\100y_combined.mupp",
   "StartTime": "2007-01-01 11:00:00",
   "EndTime": "2007-01-01 13:30:00",
   "TimeStepType": 1,
   "TimeStepFixed": 1,
   "TimeStepMin": null,
   "TimeStepMax": null,
   "OutputFolderType": 1,
   "OutputFolder": null,
   "OutputInterval": [
      {
         "OutputID": "Result_file_2",
         "SaveEvery": 30,
         "SaveEveryUnit": 1
      },
      {
         "OutputID": "Result_file_3",
         "SaveEvery": 30,
         "SaveEveryUnit": 1
      },
      {
         "OutputID": "2D dynamics",
         "SaveEvery": 300,
         "SaveEveryUnit": 5
      },
      {
         "OutputID": "2D statistics",
         "SaveEvery": 60,
         "SaveEveryUnit": 1
      },
      {
         "OutputID": "Default_Surface_runoff",
         "SaveEvery": 60,
         "SaveEveryUnit": 1
      }
   ]
});

var requestOptions = {
   method: 'POST',
   headers: myHeaders,
   body: raw
};

fetch("https://online-products.dhichina.cn/devcore/dev-core-demo-service-mpflood/api/Basic/SetModelBasicInfo", requestOptions)
   .then(response => response.text())
   .then(result => console.log(result))
   .catch(error => console.log('error', error));